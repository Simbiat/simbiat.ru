...
use App\Notification\EnoughSpace;
use App\Notification\ErrorLog;
use App\Notification\NoSpace;
    ...
    public function noSpace(): void
    {
        ...
        if (Config::$prod && $percentage < 5) {
            ...
            if (!\is_file($dir.'/noSpace.flag')) {
                ...
                if ($percentage < 5) {
                    #Send mail
                    new NoSpace()->save(SystemUser::Owner->value, ['percentage' => $percentage, 'free' => CuteBytes::bytes($free, 1024), 'total' => CuteBytes::bytes($total, 1024)], true, false, Config::ADMIN_MAIL)->send();
                    #Generate flag
            ...
            #Send mail
            new EnoughSpace()->save(SystemUser::Owner->value, ['percentage' => $percentage, 'free' => CuteBytes::bytes($free, 1024), 'total' => CuteBytes::bytes($total, 1024)], true, false, Config::ADMIN_MAIL)->send();
        }
    }
    ...
    public function dbDown(): void
    {
        ...
        if (Config::$prod && !Config::$dbup) {
            ...
            if (!\is_file($no_db_flag)) {
                #Send mail
                new DatabaseDown()->save(SystemUser::Owner->value, ['errors' => \print_r(Pool::$errors, true)], true, false, Config::ADMIN_MAIL)->send();
                #Generate flag
        ...
        if (\is_file($crash_flag)) {
            ...
            @\unlink($no_db_flag);
            #Send mail
            new DatabaseUp()->save(SystemUser::Owner->value, ['maintenance' => true, 'restored' => $result, 'error_text' => $error_text], true, false, Config::ADMIN_MAIL)->send();
            return;
        }
        if (\is_file($no_db_flag)) {
            /** @noinspection PhpUsageOfSilenceOperatorInspection Not critical, probably concurrency issue */
            ...
            #Send mail
            new DatabaseUp()->save(SystemUser::Owner->value, ['maintenance' => false], true, false, Config::ADMIN_MAIL)->send();
        }
    }
    ...
    public function errorLog(): void
    {
        ...
        if (\is_file($error_log)) {
            if (!\is_file($error_flag)) {
                #Send mail
                new ErrorLog()->save(SystemUser::Owner->value, [], true, false, Config::ADMIN_MAIL)->send();
                #Generate flag
