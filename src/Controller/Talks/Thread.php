...
class Thread extends Page
{
    ...
    protected function generate(array $path): array
    {
        ...
        if ($output_array['private']) {
            if ($output_array['author'] === SystemUser::Unknown->value && $output_array['author'] === $_SESSION['user_id']) {
        ...
        $output_array['external_links_list'] = $output_array['external_links'];
        foreach ($output_array['external_links'] as $type => $link) {
            if (Sanitize::whiteString($link['url'] ?? '')) {
                unset($output_array['external_links_list'][$type]);
            } else {
