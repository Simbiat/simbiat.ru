...
class Bic extends Page
{
    ...
    protected function generate(array $path): array
    {
        ...
        $bic = $path[0] ?? '';
        #Try to get details
        $output_array['bicdetails'] = new \App\Entity\BIC($bic)->getArray();
        ...
        if (
            \in_array($output_array['bicdetails']['Rgn'], ['ДОНЕЦКАЯ НАРОДНАЯ РЕСПУБЛИКА', 'ЗАПОРОЖСКАЯ ОБЛАСТЬ', 'ЛУГАНСКАЯ НАРОДНАЯ РЕСПУБЛИКА', 'ХЕРСОНСКАЯ ОБЛАСТЬ', 'РЕСПУБЛИКА КРЫМ', 'Г СЕВАСТОПОЛЬ'], true) ||
            \in_array($output_array['bicdetails']['Nnp'], ['Донецк', 'Луганск', 'Херсон', 'Мелитополь', 'ФЕОДОСИЯ', 'ГЕОРГИЕВКА 2'], true)
        ) {
        ...
        foreach ($output_array['bicdetails']['restrictions'] as $restriction => $details) {
            if ($details['type'] === 'account') {
                ...
                if (!empty($details['SuccessorBIC'])) {
                    #TODO: Ideally, link should be turned into an entity card. Currently it gets sanitized, so may need a better way to handle this.
                    $output_array['bicdetails']['restrictions'][$restriction]['description'] .= ' Преемник: <a href="/bictracker/bics/'.$details['SuccessorBIC']['id'].'">'.$details['SuccessorBIC']['name'].'</a>';
                }
            }
        }
        return $output_array;
