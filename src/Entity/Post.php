...
use App\Notification\NewPost;
use App\Notification\PostChange;
use App\Notification\TicketChange;
use App\Notification\TicketCreation;
...
final class Post extends Entity
{
    ...
    public function add(bool $first_post = false): array
    {
        ...
        try {
            ...
            if (
                ...
                if (
                    $thread->author === $_SESSION['user_id']
                    && $this->access_token !== ''
    ...
    private function sanitizeInput(array &$data): bool|array
    {
        ...
        if (
            empty($data['text'])
            || \preg_match('/^(<p?)\s*(<\/p>)?$/ui', $data['text']) === 1
        ) {
