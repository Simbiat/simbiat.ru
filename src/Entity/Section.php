...
use App\Notification\SectionChange;
use App\Service\Curl;
use App\Service\Errors;
use App\Service\Sanitization;
use App\Service\Search\Sections;
...
final class Section extends Entity
{
     ...
     *
     * @param bool $for_thread
     *
     * @return $this
     */
    public function setForThread(bool $for_thread): self
    {
    ...
    protected function getFromDB(): array
    {
        ...
        if ($this->id === 'top') {
            ...
            if (empty($data['parent_id'])) {
                $data['parents'] = [];
                $data['inherited_type'] = match ($this->id) {
                    '1' => 'Blog',
                    '2' => 'Forum',
                    '3' => 'Changelog',
                    '4' => 'Knowledgebase',
                ...
                $data['inherited_type'] = match ($data['parents'][0]['section_id']) {
                    1 => 'Blog',
                    2 => 'Forum',
                    3 => 'Changelog',
                    4 => 'Knowledgebase',
            ...
            if ($inherited_ownership || $data['author'] === $_SESSION['user_id']) {
                $data['owned'] = true;
            } else {
                $data['owned'] = false;
            }
            ...
            if ($data['detailed_type'] === 'Category' || $this->for_thread) {
                ...
                if (($data['detailed_type'] === 'Support' || $data['inherited_type'] === 'Support') && $_SESSION['user_id'] === SystemUser::Unknown->value) {
                    $where .= ' AND `talks__threads`.`author`!=:anonymous';
                    $bindings[':anonymous'] = [SystemUser::Unknown->value, 'int'];
                }
                if (!in_array('view_scheduled', $_SESSION['permissions'], true)) {
        ...
        if (!$this->for_thread) {
            ...
            if (!empty($data['children']['entities'])) {
                ...
                #Do not show threads created by "Unknown user"
                if ($this->id !== 'top' && ($data['detailed_type'] === 'Support' || $data['inherited_type'] === 'Support') && $_SESSION['user_id'] === SystemUser::Unknown->value) {
    ...
    protected function process(array $from_db): void
    {
        ...
        $this->type = $from_db['detailed_type'] ?? $from_db['type'] ?? 'Category';
        $this->inherited_type = $from_db['inherited_type'] ?? 'Category';
    ...
    private function sanitizeInput(array &$data, bool $edit = false): bool|array
    {
        ...
        switch ($data['type']) {
            case 2:
                ...
                if (Sanitize::whiteString($this->name)) {
        ...
        if (
            ...
            (
                ...
                Sanitize::whiteString($this->name) ||
