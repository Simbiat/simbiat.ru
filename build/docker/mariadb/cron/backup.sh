...
if [ "$WEB_SERVER_TEST" != "true" ]; then
  ...
  trap 'catch $? $LINENO' EXIT
    ...
    if [ "$1" != "0" ]; then
      ...
      echo "Error $1 occurred on $2">> /usr/local/logs/db_maintenance.flag 2>&1
      mv /usr/local/logs/db_maintenance.flag /usr/local/logs/backup_crash.flag
    fi
  }

  touch /usr/local/logs/db_maintenance.flag
  {
  ...
  rm -f /usr/local/logs/db_maintenance.flag
fi
exit