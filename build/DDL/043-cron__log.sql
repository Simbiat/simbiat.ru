...
CREATE TABLE IF NOT EXISTS `cron__log` (
  ...
  `type` varchar(30) NOT NULL DEFAULT 'Status' COMMENT 'Event type',
  `runby` varchar(30) DEFAULT NULL COMMENT 'Indicates process that was running a task',
  `sse` tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Flag to indicate whether task was being ran by SSE call',
  `task` varchar(100) DEFAULT NULL COMMENT 'Optional task ID',
  `arguments` varchar(255) DEFAULT NULL COMMENT 'Optional task arguments',
  `instance` int(10) unsigned DEFAULT NULL COMMENT 'Instance number of the task',
  `message` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT 'Message provided by the event',
  KEY `time` (`time`),
