...
CREATE TABLE IF NOT EXISTS `sys__logs` (
  ...
  KEY `log_type` (`type`),
  KEY `time` (`time` DESC),
  CONSTRAINT `audit_to_user` FOREIGN KEY (`user_id`) REFERENCES `uc__users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `log_type` FOREIGN KEY (`type`) REFERENCES `sys__log_types` (`type_id`) ON DELETE CASCADE ON UPDATE CASCADE
