...
import { AJAX_TIMEOUT } from 'Common/Constants.ts';
import { updateHistory } from "Common/Helpers.ts";
...
export class BICKeying {
  ...
  private calc(): boolean {
    if (this.form && this.result && this.bicKeySample && this.accKeySample) {
      ...
      const bicKey = String(form_data.get('bic_key') ?? '');
      const accKey = String(form_data.get('account_key') ?? '');
      ...
      void ajax(`${location.protocol}//${location.host}/api/bictracker/keying`, form_data, 'json', 'POST', AJAX_TIMEOUT, true)
        .then((response) => {
