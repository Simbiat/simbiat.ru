...
export class EditAvatars {
  ...
  private readonly avatar_file: HTMLInputElement | null = null;
  private readonly avatars_list: HTMLUListElement | null = null;
  ...
  public constructor() {
    ...
    this.avatar_file = document.querySelector('#profile_avatar_file');
    this.form = document.querySelector('#profile_avatar');
    this.avatars_list = document.querySelector('#avatars_list');
  ...
  private listen(): void {
    ...
    for (const item of document.querySelectorAll('input[id^="avatar_"]')) {
      item.addEventListener('change', (event: Event) => {
        this.setActive(event.target as HTMLInputElement);
      });
    }
    //Listen to avatar deletion
    for (const item of document.querySelectorAll('input[id^="del_"]')) {
