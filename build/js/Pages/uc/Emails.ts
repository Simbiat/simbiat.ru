import { addSnackbar, submitIntercept, empty, deleteRow } from 'Common/Helpers.ts';
...
export class Emails {
  ...
  public constructor() {
    ...
    if (this.add_mail_form) {
      ...
      document.querySelectorAll('.mail_activation')
              .forEach((item) => {
      ...
      document.querySelectorAll('[id^=subscription_checkbox_]')
              .forEach((item) => {
      ...
      document.querySelectorAll('.mail_deletion')
              .forEach((item) => {
                item.addEventListener('click', (event) => {
  ...
  private static delete(button: HTMLInputElement): void {
    ...
    buttonToggle(button);
    void ajax(`${location.protocol}//${location.host}/api/uc/emails/delete`, form_data, 'json', 'DELETE', AJAX_TIMEOUT, true).then((response) => {
        const data = response as AjaxJSONResponse;
        if (data.data === true) {
          deleteRow(button);
          Emails.blockDelete();
  ...
  private static activate(button: HTMLInputElement): void {
    ...
    form_data.set('email', email);
    buttonToggle(button);
    void ajax(`${location.protocol}//${location.host}/api/uc/emails/activate`, form_data, 'json', 'PATCH', AJAX_TIMEOUT, true).then((response) => {
        const data = response as AjaxJSONResponse;
        if (data.data === true) {
