import { addSnackbar, empty, submitIntercept } from 'Common/Helpers.ts';
...
export class EditProfile {
  ...
  private profile(auto = false): void {
    if (this.profileForm) {
      //Get form data
      const form_data = new FormData(this.profileForm);
      void ajax(`${location.protocol}//${location.host}/api/uc/profile`, form_data, 'json', 'PATCH', AJAX_TIMEOUT, true).then((response) => {
          const data = response as AjaxJSONResponse;
          if (data.data === true) {
            this.profileFormData = JSON.stringify([...form_data.entries()]);
            this.profileOnChange();
  ...
  private profileOnChange(): void {
    if (this.profileForm && this.profileSubmit) {
      ...
      this.profileSubmit.disabled = this.profileFormData === JSON.stringify([...form_data.entries()]);
  ...
  private username(): void {
    if (this.usernameForm && this.usernameSubmit) {
      //Get form data
      const form_data = new FormData(this.usernameForm);
      buttonToggle(this.usernameSubmit);
      void ajax(`${location.protocol}//${location.host}/api/uc/username`, form_data, 'json', 'PATCH', AJAX_TIMEOUT, true).then((response) => {
          const data = response as AjaxJSONResponse;
          if (data.data === true) {
