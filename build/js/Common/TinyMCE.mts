...
import type { TinyMCE as TinyMCEType, Editor, RawEditorOptions } from 'tinymce/tinymce.d.ts';

/**
 * Preparing and customizing TinyMCE forms.
 */
export class TinyMCE {
  private static tinymce_module: TinyMCEType | null = null;

  /**
   * Load TinyMCE editor.
   */
  private static async loadModule(): Promise<void> {
    if (this.tinymce_module !== null) {
      return;
    }
    ...
    // TinyMCE is a UMD bundle - it registers on window, not as a default export
    this.tinymce_module = (window as unknown as { tinymce: TinyMCEType }).tinymce;
  }

  /**
   * Allowed color codes.
   */
  private static readonly CUSTOM_COLOR_MAP = new Map<string, string>([
    ['#17141F', 'body'],
    ['#19424D', 'dark_border'],
    ['#231F2E', 'block'],
    ['#266373', 'light_border'],
   ...
   * TinyMCE settings.
   */
  private static readonly TINY_SETTINGS: RawEditorOptions = {
    ...
    color_map: [...this.CUSTOM_COLOR_MAP.entries()].flat(),
        ...
        attributes: {
          ...
          class: (vars?): string => {
            ...
            if (color_key !== null && typeof color_key !== 'undefined') {
              const mapped = this.CUSTOM_COLOR_MAP.get(color_key) ?? '';
   ...
   */
  private static toTextarea(textarea: HTMLTextAreaElement, tiny_instance: Editor): void {
   ...
   */
  private static hideInputs(): void {
    // Get dialog
   ...
   */
  public static async load(id: string, no_media = true, no_restore_on_empty = false): Promise<void> {
    if ((/^\s*$/v).exec(id)) {
      return;
    }
    await this.loadModule();
    const textarea = document.querySelector<HTMLTextAreaElement>(`#${id}`);
    if (textarea && this.tinymce_module !== null) {
      const settings = this.TINY_SETTINGS;
      ...
      const instances: Editor[] = await this.tinymce_module.init(settings);
      ...
      for (const event_type of [
        ...
        tiny_instance.on(event_type, () => {
          ...
          this.toTextarea(textarea, tiny_instance);
        });
      }
    }
  }
   ...
   */
  public static async save(id: string, textarea_only = false): Promise<void> {
    if ((/^\s*$/v).exec(id)) {
      return;
    }
    const textarea = document.querySelector<HTMLTextAreaElement>(`#${id}`);
    await this.loadModule();
    if (textarea !== null && this.tinymce_module !== null) {
      const tiny_instance = this.tinymce_module.get(id);
