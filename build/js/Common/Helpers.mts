...
export function urlCleanString(url: string): string {
    const source = document.querySelector('#tracking_query_parameters')?.textContent;
    const parsed: unknown = JSON.parse(source);

    if (!Array.isArray(parsed)) {
        throw new TypeError('tracking_query_parameters must be a JSON array');
    }
...
export function urlClean(event: ClipboardEvent): void {
    ...
    let clean_string;
    try {
        clean_string = urlCleanString(original_string ?? '');
    } catch {
        clean_string = original_string ?? '';
    }
    Input.pasteAndMove((current as HTMLInputElement), clean_string);
