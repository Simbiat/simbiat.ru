import { addSnackbar, empty, pageRefresh, submitIntercept } from 'Common/Helpers.ts';
...
import { ariaNation, buttonToggle } from 'Common/Inputs.ts';
...
export class LoginForm extends HTMLElement {
  ...
  private singInUpSubmit(): void {
    if (this.login_form) {
      ...
      void ajax(`${location.protocol}//${location.host}/api/uc/${String(form_data.get('signinup[type]'))}`, form_data, 'json', 'POST', AJAX_TIMEOUT, true).then((response) => {
          const data = response as AjaxJSONResponse;
          if (data.data === true) {
            if (form_data.get('signinup[type]') === 'remind') {
