...
use Simbiat\Twig\EnvironmentGenerator;
...
class HomePage
{
    ...
    private function init(): void
    {
        ...
        try {
            ...
            try {
                ...
                if ($uri[0] !== 'api') {
                    \Simbiat\Config::$links = array_merge(\Simbiat\Config::$links, [
                        ['rel' => 'stylesheet preload', 'href' => '/assets/styles/' . filemtime(\Simbiat\Config::$cssDir.'/app.css') . '.css', 'as' => 'style'],
                        ['rel' => 'preload', 'href' => '/assets/app.' . filemtime(\Simbiat\Config::$jsDir.'/app.js') . '.js', 'as' => 'script'],
                    ]);
                }
                Links::links(\Simbiat\Config::$links);
                ...
                if (!empty($_SERVER['CADDY_HTTP_ERROR_php'])) {
                    self::$http_error = ['http_error' => $_SERVER['CADDY_HTTP_ERROR_php']];
                }
    ...
    public function canonical(): void
    {
        ...
        self::$canonical = 'https://'.(preg_match('/^[a-z\d\-_~]+\.[a-z\d\-_~]+$/iu', \Simbiat\Config::$http_host) === 1 ? 'www.' : '').\Simbiat\Config::$http_host.($_SERVER['SERVER_PORT'] !== '443' ? ':'.$_SERVER['SERVER_PORT'] : '').'/'.self::$canonical;
    ...
    final public function twigProc(array $twigVars = [], bool $cache = false): bool
    {
        if ($cache) {
            ...
            if (\Simbiat\Config::$PROD && !empty($twigVars['cacheAge']) && is_numeric($twigVars['cacheAge']) && empty($twigVars['http_error']) && self::$method === 'GET') {
