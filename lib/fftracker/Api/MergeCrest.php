...
class MergeCrest extends Api
{
    ...
    protected function genData(array $path): array
    {
        ...
        if (!is_file(Config::$crestsComponents.'emblems/'.mb_strtolower(mb_substr($_POST['crest_emblem'], 0, 3, 'UTF-8'), 'UTF-8').'/'.$_POST['crest_emblem'])) {
        ...
        if (!empty($_POST['crest_frame'])) {
            ...
            if (!is_file(Config::$crestsComponents.'frames/'.$_POST['crest_frame'])) {
        ...
        if (!empty($_POST['crest_background'])) {
            ...
            if (!is_file(Config::$crestsComponents.'backgrounds/'.mb_strtolower(mb_substr($_POST['crest_background'], 0, 3, 'UTF-8'), 'UTF-8').'/'.$_POST['crest_background'])) {
                return ['http_error' => 400, 'reason' => 'Frame file not found'];
            }
            $images[0] = $_POST['crest_background'];
        }
        $location = Entity::crestToFavicon($images);
        if (empty($location)) {
