...
namespace Simbiat\Website\Talks\Pages;
...
class Section extends Page
{
    ...
    {
        #Allow `blob:`
        @header('content-security-policy: upgrade-insecure-requests; default-src \'self\'; child-src \'self\'; connect-src \'self\'; font-src \'self\'; frame-src \'self\'; img-src \'self\' blob:; manifest-src \'self\'; media-src \'self\'; object-src \'none\'; script-src \'report-sample\' \'self\'; script-src-elem \'report-sample\' \'self\'; script-src-attr \'none\'; style-src \'report-sample\' \'self\'; style-src-elem \'report-sample\' \'self\'; style-src-attr \'none\'; worker-src \'self\'; base-uri \'self\'; form-action \'self\'; frame-ancestors \'self\'; navigate-to https:;');
        #Sanitize ID
        $id = $path[0] ?? 'top';
        ...
        $outputArray = (new \Simbiat\Website\Talks\Entities\Section($id))->getArray();
        ...
        if ($outputArray['owned'] || in_array('addSections', $_SESSION['permissions'])) {
            $outputArray['section_types'] = \Simbiat\Website\Talks\Entities\Section::getSectionTypes($outputArray['inheritedType']);
        }
        #Get stuff for threads
        if ($outputArray['owned'] || in_array('canPost', $_SESSION['permissions'])) {
            $outputArray['thread_languages'] = \Simbiat\Website\Talks\Entities\Thread::getLanguages();
            $outputArray['thread_link_types'] = \Simbiat\Website\Talks\Entities\Thread::getAltLinkTypes();
        }
        if ($this->editMode) {
