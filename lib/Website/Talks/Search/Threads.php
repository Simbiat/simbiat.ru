...
namespace Simbiat\Website\Talks\Search;
...
class Threads extends Search
{
    ...
    #List of fields
    protected string $fields = '`talks__threads`.`threadid` as `id`, `talks__threads`.`sectionid`, `talks__threads`.`name`, `language`, `pinned`, `talks__threads`.`system`, `talks__threads`.`private`, `talks__threads`.`closed`, `talks__threads`.`created`, `talks__threads`.`createdby`, `talks__threads`.`updated`, `talks__threads`.`updatedby`, `talks__threads`.`lastpost`, `talks__threads`.`lastpostby`, `for_creation`.`username` as `createdby_name`, `for_update`.`username` as `updatedby_name`, `for_lastpost`.`username` as `lastpostby_name`, `talks__types`.`type` as `detailedType`, `ogimage`, (SELECT `postid` FROM `talks__posts` WHERE `talks__posts`.`threadid`=`talks__threads`.`threadid` ORDER BY `createdby` ASC LIMIT 1) as `firstPost`, CONCAT(\'/assets/images/uploaded/\', SUBSTRING(`sys__files`.`fileid`, 1, 2), \'/\', SUBSTRING(`sys__files`.`fileid`, 3, 2), \'/\', SUBSTRING(`sys__files`.`fileid`, 5, 2), \'/\', `sys__files`.`fileid`, \'.\', `sys__files`.`extension`) as `icon`';
