...
class Cron
{
    ...
    {
        try {
            return HomePage::$dbController->query('UPDATE `talks__posts` SET `updated`=`updated`, `locked`=1 WHERE `created` <= DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 DAY) AND `locked`=0;');
            ...
            foreach ($allFiles as $file) {
                ...
                if (!is_dir($file) && preg_match('/.*\.gitignore$/ui', $file) !== 1) {
                    ...
                    if (!in_array(pathinfo($file, PATHINFO_FILENAME), $dbFiles)) {
                        ...
                        #Log the removal
                        Security::log('File upload', 'Automatically deleted file', basename($file), userid: \Simbiat\Website\Config::userIDs['System user']);
                        #Remove the file
                        @unlink($file);
