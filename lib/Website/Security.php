...
class Security
{
    ...
    #Time of 0.005 seems a good middle ground to keep average to allow speedy webpages load (with ~500-600ms on pages that are relatively small, but do have some computation)
    ...
    public static function log(string $type, string $action, mixed $extras = NULL, ?int $userid = null): bool
    {
        ...
        $ip = $_SESSION['IP'] ?? null;
        #Get username
        $userid = $_SESSION['userid'] ?? $userid ?? Website\Config::userIDs['Unknown user'];
        #Get User Agent
