    ...
    protected function generate(array $path): array
    {
        ...
        switch ($path[0]) {
            ...
            case 'threads':
                ...
                $query = 'SELECT CONCAT(\'talks/threads/\', `thread_id`) AS `loc`, `updated` AS `lastmod`, `name` FROM `talks__threads` WHERE `private`=0 AND `talks__threads`.`published`<=CURRENT_TIMESTAMP(6) ORDER BY `name` LIMIT '.$start.', '.$this->max_elements;
                break;
            case 'users':
