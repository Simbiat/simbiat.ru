    ...
    private function feed(array $uri, string $format = 'atom'): array
    {
        ...
        if (!empty($uri[0])) {
            ...
            if (!empty($title)) {
                ...
                if (!empty($query)) {
                    if ($format === 'atom') {
                        Atom::Atom(Config::siteName.': '.$title, HomePage::$dbController->selectAll($query), feed_settings: $settings);
                    } elseif ($format === 'rss') {
                        RSS::RSS(Config::siteName.': '.$title, HomePage::$dbController->selectAll($query), feed_settings: $settings);
                    }
                }
            }
        }
