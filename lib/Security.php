...
class Security
{
    ...
    {
        return password_hash($password, PASSWORD_ARGON2ID, \Simbiat\Config::$argonSettings);
    }
    ...
    public static function encrypt(#[\SensitiveParameter] string $data): string
    {
        ...
        $encrypted = openssl_encrypt($data, 'AES-256-GCM', hex2bin(\Simbiat\Config::$aesSettings['passphrase']), OPENSSL_RAW_DATA, $iv, $tag);
    ...
    public static function decrypt(string $data): string
    {
        ...
        $data = substr($data, 28);
        return openssl_decrypt($data, 'AES-256-GCM', hex2bin(\Simbiat\Config::$aesSettings['passphrase']), OPENSSL_RAW_DATA, $iv, $tag);
    }
    ...
    public static function argonCalc(bool $forceRefresh = false): array
    {
        ...
        if (is_file(\Simbiat\Config::$securityCache. '/argon.json') && !$forceRefresh) {
            #Read the file
            $argon = json_decode(file_get_contents(\Simbiat\Config::$securityCache. '/argon.json'), true);
            if (is_array($argon)) {
    ...
    {
        if (is_file(\Simbiat\Config::$securityCache. '/aes.json') && !$forceRefresh) {
            $aes = json_decode(file_get_contents(\Simbiat\Config::$securityCache.'/aes.json'), true);
            if (is_array($aes)) {
    ...
    public static function log(string $type, string $action, mixed $extras = NULL, ?int $userid = null): bool
    {
        ...
        $ip = $_SESSION['IP'] ?? null;
        #Get username
        $userid = $_SESSION['userid'] ?? $userid ?? \Simbiat\Config::userIDs['Unknown user'];
        #Get User Agent
