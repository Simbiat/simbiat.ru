...
class Posts extends Search
{
    ...
    #List of fields
    protected string $fields = '`postid` as `id`, `talks__posts`.`threadid`, `talks__threads`.`name`, `talks__types`.`type` as `detailedType`, `replyto`, `locked`, `talks__posts`.`system`, `talks__posts`.`created`, `talks__posts`.`createdby`, `talks__posts`.`updated`, `talks__posts`.`updatedby`, `text`, `for_creation`.`username` as `createdby_name`, `for_update`.`username` as `updatedby_name`, COALESCE((SELECT CONCAT(\'/img/avatars/\', SUBSTRING(`sys__files`.`fileid`, 1, 2), \'/\', SUBSTRING(`sys__files`.`fileid`, 3, 2), \'/\', SUBSTRING(`sys__files`.`fileid`, 5, 2), \'/\', `sys__files`.`fileid`, \'.\', `sys__files`.`extension`) AS `avatar` FROM `uc__avatars` LEFT JOIN `sys__files` ON `uc__avatars`.`fileid`=`sys__files`.`fileid` WHERE `uc__avatars`.`userid`=`for_creation`.`userid` AND `uc__avatars`.`current`=1), \'/img/avatar.svg\') AS `avatar`, (SELECT COUNT(*) FROM `talks__likes` WHERE `talks__likes`.`postid`=`talks__posts`.`postid` AND `likevalue`<0) as `dislikes`, (SELECT COUNT(*) FROM `talks__likes` WHERE `talks__likes`.`postid`=`talks__posts`.`postid` AND `likevalue`>0) as `likes`';
    #Optional JOIN string, in case it's needed
    protected string $join = 'LEFT JOIN `uc__users` as `for_creation` ON `talks__posts`.`createdby`=`for_creation`.`userid` LEFT JOIN `uc__users` as `for_update` ON `talks__posts`.`updatedby`=`for_update`.`userid` INNER JOIN `talks__threads` ON `talks__posts`.`threadid`=`talks__threads`.`threadid` INNER JOIN `talks__sections` ON `talks__sections`.`sectionid`=`talks__threads`.`sectionid` INNER JOIN `talks__types` ON `talks__sections`.`type`=`talks__types`.`typeid`';
