...
class Statistics
{
    ...
    private function getTimelines(array &$data, Controller $dbCon): void
    {
        ...
        $data['timelines'] = $dbCon->selectAll(
            'SELECT `registered` AS `date`, COUNT(*) AS `count`, \'characters_registered\' as `type` FROM `ffxiv__character` GROUP BY `date`
                            ...
                            UNION
                            SELECT DATE(`updated`) AS `date`, COUNT(*) AS `count`, \'characters_updated\' as `type` FROM `ffxiv__character` WHERE `deleted` IS NULL AND `registered`<>DATE(`updated`) GROUP BY `date`
                            UNION
                            ...
                            UNION
                            SELECT DATE(`updated`) AS `date`, COUNT(*) AS `count`, \'free_companies_updated\' as `type` FROM `ffxiv__freecompany` WHERE `deleted` IS NULL AND `registered`<>DATE(`updated`) GROUP BY `date`
                            UNION
                            ...
                            UNION
                            SELECT DATE(`updated`) AS `date`, COUNT(*) AS `count`, \'pvp_teams_updated\' as `type` FROM `ffxiv__pvpteam` WHERE `deleted` IS NULL AND `registered`<>DATE(`updated`) GROUP BY `date`
                            UNION
                            ...
                            UNION
                            SELECT DATE(`updated`) AS `date`, COUNT(*) AS `count`, \'linkshells_updated\' as `type` FROM `ffxiv__linkshell` WHERE `deleted` IS NULL AND `registered`<>DATE(`updated`) GROUP BY `date`;'
        );
    ...
    private function getBugs(array &$data, Controller $dbCon): void
    {
        ...
        $duplicateNames = $dbCon->selectAll(
            'SELECT \'character\' AS `type`, `characterid` AS `id`, `name`, `avatar` as `icon`, `userid`, NULL as `crest_part_1`, NULL as `crest_part_2`, NULL as `crest_part_3`, `server`, `datacenter` FROM `ffxiv__character` as `chartable` LEFT JOIN `ffxiv__server` ON `ffxiv__server`.`serverid`=`chartable`.`serverid` WHERE `deleted` is NULL AND (SELECT COUNT(*) as `count` FROM `ffxiv__character` WHERE `ffxiv__character`.`name`=`chartable`.`name` AND `ffxiv__character`.`serverid`=`chartable`.`serverid` AND `deleted` is NULL)>1
                            UNION ALL
