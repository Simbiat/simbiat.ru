...
class Achievement extends Entity
{
    ...
    protected function getFromDB(): array
    {
        ...
        $data['characters'] = $this->dbController->selectAll('SELECT * FROM (SELECT \'character\' AS `type`, `'.self::dbPrefix.'character`.`characterid` AS `id`, `'.self::dbPrefix.'character`.`name`, `'.self::dbPrefix.'character`.`characterid` AS `icon` FROM `'.self::dbPrefix.'character_achievement` LEFT JOIN `'.self::dbPrefix.'character` ON `'.self::dbPrefix.'character`.`characterid` = `'.self::dbPrefix.'character_achievement`.`characterid` WHERE `'.self::dbPrefix.'character_achievement`.`achievementid` = :id ORDER BY `'.self::dbPrefix.'character_achievement`.`time` DESC LIMIT 50) t ORDER BY `name`', [':id'=>$this->id]);
    ...
    public function getFromLodestone(): string|array
    {
        ...
        $altChars = $this->dbController->selectColumn(
            'SELECT `characterid` FROM `'.self::dbPrefix.'character_achievement` WHERE `achievementid`=:ach ORDER BY `time` DESC;',
            [
                ':ach' => $this->id,
            ]
        );
