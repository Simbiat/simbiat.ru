...
class PvPTeam extends Entity
{
    ...
    protected function getFromDB(): array
    {
        ...
        #Get members
        $data['members'] = $this->dbController->selectAll('SELECT `'.self::dbPrefix.'pvpteam_character`.`characterid`, `'.self::dbPrefix.'character`.`pvp_matches` AS `matches`, `'.self::dbPrefix.'character`.`name`, `'.self::dbPrefix.'character`.`avatar`, `'.self::dbPrefix.'pvpteam_rank`.`rank`, `'.self::dbPrefix.'pvpteam_rank`.`pvprankid` FROM `'.self::dbPrefix.'pvpteam_character` LEFT JOIN `'.self::dbPrefix.'pvpteam_rank` ON `'.self::dbPrefix.'pvpteam_rank`.`pvprankid`=`'.self::dbPrefix.'pvpteam_character`.`rankid` LEFT JOIN `'.self::dbPrefix.'character` ON `'.self::dbPrefix.'pvpteam_character`.`characterid`=`'.self::dbPrefix.'character`.`characterid` WHERE `'.self::dbPrefix.'pvpteam_character`.`pvpteamid`=:id AND `current`=1 ORDER BY `'.self::dbPrefix.'pvpteam_character`.`rankid` , `'.self::dbPrefix.'character`.`name` ', [':id'=>$this->id]);
