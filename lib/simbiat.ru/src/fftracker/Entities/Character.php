...
class Character extends Entity
{
    ...
    protected function getFromDB(): array
    {
        ...
        $data['groups'] = $this->dbController->selectAll(
            '(SELECT \'freecompany\' AS `type`, 0 AS `crossworld`, `'.self::dbPrefix.'freecompany_character`.`freecompanyid` AS `id`, `'.self::dbPrefix.'freecompany`.`name` as `name`, `current`, `'.self::dbPrefix.'freecompany_character`.`rankid`, `'.self::dbPrefix.'freecompany_rank`.`rankname` AS `rank`, `'.self::dbPrefix.'freecompany`.`crest` AS `icon` FROM `'.self::dbPrefix.'freecompany_character` LEFT JOIN `'.self::dbPrefix.'freecompany` ON `'.self::dbPrefix.'freecompany_character`.`freecompanyid`=`'.self::dbPrefix.'freecompany`.`freecompanyid` LEFT JOIN `'.self::dbPrefix.'freecompany_rank` ON `'.self::dbPrefix.'freecompany_rank`.`freecompanyid`=`'.self::dbPrefix.'freecompany`.`freecompanyid` AND `'.self::dbPrefix.'freecompany_character`.`rankid`=`'.self::dbPrefix.'freecompany_rank`.`rankid` WHERE `characterid`=:id)
            UNION ALL
