...
class User extends Entity
{
    ...
    protected function getFromDB(): array
    {
        ...
        if (in_array($this->id, [\Simbiat\Config::userIDs['Unknown user'], \Simbiat\Config::userIDs['System user'], \Simbiat\Config::userIDs['Deleted user']], true)) {
            #System users need to be treated as not activated
            $dbData['activated'] = false;
        } else {
            $dbData['activated'] = !in_array(\Simbiat\Config::groupsIDs['Unverified'], $dbData['groups'], true);
        }
    ...
    public function rememberMe(string $cookieId = ''): void
    {
        try {
            ...
            if (HomePage::$dbController !== null && ((!empty($_SESSION['userid']) && !in_array($_SESSION['userid'], [\Simbiat\Config::userIDs['Unknown user'], \Simbiat\Config::userIDs['System user'], \Simbiat\Config::userIDs['Deleted user']], true)) || !empty($this->id))) {
    ...
    public function passValid(#[\SensitiveParameter] string $password, #[\SensitiveParameter] string $hash): bool
    {
        ...
        try {
            if (password_verify($password, $hash)) {
                ...
                if (password_needs_rehash($hash, PASSWORD_ARGON2ID, \Simbiat\Config::$argonSettings)) {
    ...
    public function remove(bool $hard = false): bool
    {
        ...
        if (in_array($this->id, \Simbiat\Config::userIDs, true)) {
            return false;
        }
        try {
            ...
            if ($hard) {
                ...
                $queries = [
                    [
                        ...
                        [':userid' => [$this->id, 'int'], ':deleted' => [\Simbiat\Config::userIDs['Deleted user'], 'int']]
                    ],
                    [
                ...
                $queries = [
                    ...
                    [
                        ...
                        [
                            ':userid' => [$this->id, 'int'],
                            ':groupid' => [\Simbiat\Config::groupsIDs['Deleted'], 'int'],
                        ]
                    ],
                ];
            }
        ...
        #Log
        Security::log('User removal', 'Removal', ['userid' => $this->id, 'hard' => $hard, 'result' => $result], ($hard ? \Simbiat\Config::userIDs['Deleted user'] : $this->id));
        return $result;
    }
}
