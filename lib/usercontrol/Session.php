...
class Session implements \SessionHandlerInterface, \SessionIdInterface, \SessionUpdateTimestampHandlerInterface
{
    ...
    private function dataRefresh(array &$data): void
    {
        ...
        try {
            ...
            if (empty($data['userid'])) {
                $data['userid'] = \Simbiat\Config::userIDs['Unknown user'];
    ...
    {
        try {
            if (HomePage::$dbController->query('DELETE FROM `uc__sessions` WHERE `time` <= DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL :life SECOND) OR `userid` IN ('.\Simbiat\Config::userIDs['System user'].', '.\Simbiat\Config::userIDs['Deleted user'].');', [':life' => [$max_lifetime, 'int']])) {
